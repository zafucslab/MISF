...
while True:
    ...
    # 依据MATLAB测量数据重建无畸变图片
    img1_rectified = cv2.remap(imgL, config.left_map1, config.left_map2, cv2.INTER_LINEAR)
    img2_rectified = cv2.remap(imgR, config.right_map1, config.right_map2, cv2.INTER_LINEAR)
    ...
    stereo.setROI1(config.validPixROI1)
    stereo.setROI2(config.validPixROI2)
    ...
    threeD = cv2.reprojectImageTo3D(disparity, config.Q, handleMissingValues=True)  # 计算三维坐标数据值
    threeD = threeD * 16
