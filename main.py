...
def detect(save_img=False):
    ...
    classify = False
    if classify:
        modelc = load_classifier(name='mobilenetv2', n=2)  # initialize
                ...
                if dataset.mode == 'image':
                    ...
                    if vid_path != save_path:  # new video
                        ...
                        if vid_cap:  # video
                            ...
                            save_path += '.jpg'
                        vid_writer = cv2.VideoWriter(save_path, cv2.VideoWriter_fourcc(*'jpg'), fps, (w, h))
                    vid_writer.write(im0)
                    cv2.namedWindow("picture", cv2.WINDOW_NORMAL)
                    cv2.resizeWindow("picture", 1280, 480)
                    cv2.moveWindow("picture", 0, 0)
                    cv2.imshow("picture", im0)
                    cv2.waitKey(1)
